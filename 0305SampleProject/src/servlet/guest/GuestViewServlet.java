...
request.setCharacterEncoding("utf-8");
		...
		for(int i=0; i<cookies.length; i++) {
			if(cookies[i].getName().equals("board")) {
				...
				if(cookies[i].getValue().contains("G"+request.getParameter("idx"))) {
					isReadCnt=true;
					break;
				}
			}
		...
		if(!isReadCnt) {
			...
			String value = null;
			if(boardCokies == null) {
				value = "G"+request.getParameter("idx");
			}else {
				value = boardCokies+"G"+request.getParameter("idx");
			}
		...
		GuestVO vo = dao.getOnePost(Integer.valueOf(request.getParameter("idx")));
